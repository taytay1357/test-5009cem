...
Deno.test("checking the home screen displays correct data", async (test) => {
    ...
    const data = await pageNew.$$eval('p', (nodes) => {
        for(const node of nodes) if(node.innerText.includes('a') || node.innerText.includes('A') || node.innerText.includes('e') || node.innerText.includes('E')) return true
        return false
    })
...
Deno.test("checking that all books have an add cart button", async (test) => {
    ...
    const button = await pageNew.$$eval('input', (nodes) =>{
        for(const node of nodes) if(node.innerText.includes('Add to cart')) return true
        return false
    })
...
Deno.test("checking that the number of items in the cart is displayed", async (test) => {
    const browser = await puppeteer.launch({ headless: false });
    const pageNew = await browser.newPage();
    await pageNew.goto(url, { waitUntil: "networkidle0"})
    await pageNew.click('a[href="/login"]')
    ...
    const value = await pageNew.$$eval('p', (nodes) => {
        for(const node of nodes) if(typeof node.innerText === 'number') return true; break
        return false
    })
...
Deno.test("checking that the total cost of cart is displayed", async (test) => {
    ...
    await pageNew.keyboard.press("Enter");
    const value = await pageNew.$$eval('p', (nodes) => {
        for(const node of nodes) if(node.innerText.includes('£')) return true; break
        return false
    })
...
Deno.test("checking that there is a cart icon on the users page", async (test) => {
    const browser = await puppeteer.launch({ headless: false });
    const pageNew = await browser.newPage();
    await pageNew.goto(url, { waitUntil: "networkidle0"})
    await pageNew.click('a[href="/login"]')
...
Deno.test("checking that the add cart button works", async (test) => {
    ...
    const data = await pageNew.$eval('p', nodes => {
        for (const node of nodes) if(node.innerText.includes('a') || node.innerText.includes('e') ) return true; break
        return false
    })
    await assertEquals(heading, "Your Cart", 'does not redirect you to correct page') // THIS ASSERTS WHETHER IT SENDS YOU TO CORRECT PAGE
...
Deno.test("checking that user can navigate to their cart", async (test) => {
    ...
    await assertEquals(heading, "Your Cart", 'system does not send you to correct page')
})