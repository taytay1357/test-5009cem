...
Deno.test("checking that stock levels screen displays correct data", async (test) => {
    ...
    const data = await pageNew.$$eval("p", (nodes) => {
        for(const node of nodes) if (node.innerText.includes('NAME') || node.innerText.includes('ISBN') || node.innerText.includes('QUANTITY')) return true
        return false
    })
