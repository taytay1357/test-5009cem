...
Deno.test({
    name: 'there is a record for the authorised user in the database', 
    fn: async (test) => {
        await test.step({
            name: "dataset 1",
            fn: async () => {
        ...
        await test.step({
            ...
            fn: async () => {
                // ARRANGE
                const data = {authorised: "testdummy2", isbn: "9783806741360"}
                // ACT
                const outcome = await addCart(data)
                const sql = `DELETE FROM cart WHERE FK_isbn="${data.isbn}" AND FK_user_id="${data.authorised}"`
                await db.query(sql)
        ...
        await test.step({
            ...
            fn: async () => {
                // ARRANGE
                const data = {authorised: "testdummy3", isbn: "9788854172388"}
                // ACT
                const outcome = await addCart(data)
                const sql = `DELETE FROM cart WHERE FK_isbn="${data.isbn}" AND FK_user_id="${data.authorised}"`
                await db.query(sql)
...
Deno.test({
    name: 'there is no record for the username in the database',
    fn: async () => {
        // ARRANGE
...
Deno.test({
    name: 'data passed is not undefined and there is a record for the username',
    fn: async () => {
        // ARRANGE 
        const data = "user1"
        // ACT
...
Deno.test('record is isbn number and not a * and wants to delete that specific isbn', async (t) => {
    await t.step({
        name: "dataset 1",
        fn: async () => {
            // ARRANGE
    ...
    await t.step({
        ...
        fn: async () => {
            // ARRANGE
        const data = {record: "9780140430721", authorised: "testdummy2"}
        let sql = `SELECT id FROM accounts WHERE user="${data.authorised}"`
        const records = await db.query(sql)
        const username = records[0].id
        sql = `INSERT INTO cart (FK_user_id, FK_isbn) VALUES ("${username}","${data.record}")`
...
Deno.test({
    name: 'record is a * and not an isbn number and wants to delete all cart',
    fn: async () => {
        // ARRANGE
