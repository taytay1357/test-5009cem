import { addCart, deleteCart, getCart } from "./cart.js";
import {
  assertEquals,
  assertThrows,
  fail,
...
Deno.test({
  ...
  fn: async () => {
    ...
    const sql =
      `DELETE FROM cart WHERE FK_isbn="${data.isbn}" AND FK_user_id="${data.authorised}"`;
    await db.query(sql);
    // ASSERT
...
Deno.test({
  name:
    "data passed is not undefined and there is a record for the username [1]",
  fn: async () => {
    // ARRANGE
    ...
    assertEquals(
      outcomeType,
      "object",
      "getCart doesnt return the correct type",
    );
  },
  sanitizeResources: false,
  sanitizeOps: false,
...
Deno.test({
  name:
    "data passed is not undefined and there is a record for the username [2]",
  fn: async () => {
    // ARRANGE
...
Deno.test({
  name:
    "data passed is not undefined and there is a record for the username [3]",
  fn: async () => {
    // ARRANGE
...
Deno.test({
  name:
    "record is isbn number and not a * and wants to delete that specific isbn [1]",
  fn: async () => {
    // ARRANGE
    ...
    sql =
      `INSERT INTO cart (FK_user_id, FK_isbn) VALUES ("${username}","${data.record}")`;
    await db.query(sql);
    // ACT
...
Deno.test({
  name:
    "record is isbn number and not a * and wants to delete that specific isbn [2]",
  fn: async () => {
    // ARRANGE
...
Deno.test({
  name:
    "record is isbn number and not a * and wants to delete that specific isbn [3]",
  fn: async () => {
    // ARRANGE
...
Deno.test({
  ...
  fn: async () => {
    ...
    const data = {
      isbn: "9780099549482",
      record: "*",
      authorised: "testdummy",
    };
    let sql = `SELECT id FROM accounts WHERE user="${data.authorised}"`;
    const records = await db.query(sql);
    const username = records[0].id;
    sql =
      `INSERT INTO cart (FK_user_id, FK_isbn) VALUES ("${username}","${data.isbn}")`;
    await db.query(sql);
    // ACT
    const outcome = await deleteCart(data);
    // ASSERT
